...
func TestMakeChannelServiceAddress(t *testing.T) {
	if want, got := "my-test-kc-kn-channel", channel.MakeChannelServiceName(kcName); want != got {
		t.Errorf("Want: %q got %q", want, got)
	}
}
	...
	want := &corev1.Service{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				channel.MessagingRoleLabel: "kafka-channel",
			},
...
func TestMakeServiceWithExternal(t *testing.T) {
	...
	got, err := channel.MakeK8sService(imc, &args, channel.ExternalService(testDispatcherNS, testDispatcherName))
	if err != nil {
		t.Fatalf("Failed to create new service: %s", err)
	}
